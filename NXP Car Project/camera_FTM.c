...
int * findEdge(void) {
	...
	int i,leftEdgeIndex,rightEdgeIndex;
    
	//Savitzky-Golay Filter and derivative
	for (i=0; i<128; i++){
		
		if (i < 2){
			if (i == 0){
				smooth_data[i] = ((-3.0/35.0)*(double)line[i]) + ((12.0/35.0)*(double)line[i])
                               + ((17.0/35.0)*(double)line[i]) + ((12.0/35.0)*(double)line[i+1])
                               + ((-3.0/35.0)*(double)line[i+2]);
               derivative[i] = 0.0;
		  } else {
              smooth_data[i] = ((-3.0/35.0)*(double)line[i-1]) + ((12.0/35.0)*(double)line[i-1])
                             + ((17.0/35.0)*(double)line[i]) + ((12.0/35.0)*(double)line[i+1])
                             + ((-3.0/35.0)*(double)line[i+2]);
		  }	
		} else if (i < 126){
            smooth_data[i] = ((-3.0/35.0)*(double)line[i-2]) + ((12.0/35.0)*(double)line[i-1])
                           + ((17.0/35.0)*(double)line[i]) + ((12.0/35.0)*(double)line[i+1])
                           + ((-3.0/35.0)*(double)line[i+2]);
            derivative[i-1] = (double) smooth_data[i]-smooth_data[i-2];
    } else if (i == 126){
            smooth_data[i] = ((-3.0/35.0)*(double)line[i-2]) + ((12.0/35.0)*(double)line[i-1])
                           + ((17.0/35.0)*(double)line[i]) + ((12.0/35.0)*(double)line[i+1])
                           + ((-3.0/35.0)*(double)line[i+1]);
        derivative[i-1] = (double) smooth_data[i]-smooth_data[i-2];
		} else {
            smooth_data[i] = ((-3.0/35.0)*(double)line[i-2]) + ((12.0/35.0)*(double)line[i-1])
                           + ((17.0/35.0)*(double)line[i]) + ((12.0/35.0)*(double)line[i])
                           + ((-3.0/35.0)*(double)line[i]);
        derivative[i-1] = (double) smooth_data[i]-smooth_data[i-2];
		}
		average += smooth_data[i];
	}
     derivative[127] = 0.0;
...
//   
    ...
    leftEdgeIndex = 0;     //highest slope/derivative
    rightEdgeIndex = 0;    //lowest slope/derivative
    
    //find edges
    for (i=0; i<128; i++){
        if (derivative[i] > derivative[leftEdgeIndex]) {
            leftEdgeIndex = i;
        }
        if (derivative[i] < derivative[rightEdgeIndex]) {
            rightEdgeIndex = i;
        }
    }
    edgeList[0] = leftEdgeIndex;
    edgeList[1] = rightEdgeIndex;
